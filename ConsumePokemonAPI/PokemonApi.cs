...
namespace ConsumePokemonAPI
{
    ...
    public class PokemonApi
    {
        ...
        public String GetPokemon(string pokemonName, string CaminhoNome)
        {
            //Inicaliza HttpClient e Realiza definições iniciais
            ...
            string pokemonData = string.Empty;
            string pokemonData02 = string.Empty;

            // Busca resultado no site para Pokemon Escolhido
            HttpResponseMessage response = client.GetAsync(pokemonName.ToLower()).Result;

            //Verifica retorno com sucesso
            if (response.IsSuccessStatusCode)
            {

                // recebe retorno dapesquisa
                var jresponse = response.Content.ReadAsStringAsync().Result;

                // converte retorno
                var listpokemon = JsonConvert.DeserializeObject<Pokemon>(jresponse);

                // atribuição dos valores Id|Name|Height|Weight|Abilities|Type|Front_Default|
                // Valores de Id|Name|Height|Weight|

                pokemonData = listpokemon.Id + "|" + listpokemon.Name + "|" + listpokemon.Height + "|" + listpokemon.Weight + "|";

                // Adiciona Abilities

                foreach (var ESpe in listpokemon.Abilities)
                {
                   
                    pokemonData02 = pokemonData02 + ESpe.AbilityAbility.Name + ";";

                }

                pokemonData = pokemonData + pokemonData02 + "|" ;

                pokemonData02 = String.Empty;
                
                // Valore de Type
                foreach (var EType in listpokemon.Types)
                {
                    pokemonData02 = pokemonData02 + EType.Type.Name + ";";
                }

                pokemonData = pokemonData + pokemonData02 + "|";

                // Adiciona Front_Default
                pokemonData = pokemonData + listpokemon.Sprites2.FrontDefault?.ToString() + "|";

/*
                if (!Directory.Exists( Directory.GetCurrentDirectory() + "\\A_01\\"))
                  {

                    Directory.CreateDirectory(Directory.GetCurrentDirectory() + "\\A_01\\");
                }
*/
                var p_File = CaminhoNome + listpokemon.Id + ".png";
                //Console.WriteLine(p_File);

                var teste = DownPokemonFile(listpokemon.Sprites2.FrontDefault?.ToString(), p_File);
         ...
         * 
         ***********************************************************************/
        public async Task<String> GetPokemonAsync(string pokemonName, string CaminhoNome)
        {
            try
            {
                //Inicaliza HttpClient e Realiza definições iniciais            
                ...
                // Variaveis auxiliares
                string pokemonData = string.Empty;
                string pokemonData2 = string.Empty;
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    // Valores de Abilities                              
                    Parallel.ForEach(listpokemon.Abilities, Data01 =>
                    {

                        pokemonData2 = pokemonData2 + Data01.AbilityAbility.Name + ";";

                    });


                    // Adiciona Abilities
                    pokemonData = pokemonData + pokemonData2 + "|";

                    pokemonData2 = String.Empty;

                    // Valore de Type
                    Parallel.ForEach(listpokemon.Types, Data02 =>
                    {
                        pokemonData2 = pokemonData2 + Data02.Type.Name + ";";
        ...
        public async Task<Boolean> DownPokemonFile(string Str_Url, string Str_File)
        {
           
            /*
            var httpClient = new HttpClient();
            var responseStream = await httpClient.GetStreamAsync(Str_Url);
            // using var fileStream = new FileStream( "C:\Z_Dev\" , FileMode.Create);]

            Console.Write(Directory.GetCurrentDirectory());

           using var fileStream = new FileStream(Directory.GetCurrentDirectory(), FileMode.Create);
            responseStream.CopyTo(fileStream);
          */  

        ///    Directory.GetCurrentDirectory()
           // WebClient webClient = new WebClient();
           //  webClient.DownloadFile(Str_Url, @"c:\myfile.png");

             WebClient webClient = new WebClient();
            //  webClient.DownloadFileCompleted += new AsyncCompletedEventHandler(Completed);
            //  webClient.DownloadProgressChanged += new DownloadProgressChangedEventHandler(ProgressChanged);
            webClient.DownloadFileAsync(new Uri(Str_Url), Str_File);
        ...
        /*****************************************************************************
         * 
         * Definião das Class Pokemon (Json to C#)
         * 
         ******************************************************************************/
        // Classs 
        public class Pokemon
        {
            ...
            public Sprites Sprites2 { get; set; }
